    ...
    <div class="card card-outline card-primary shadow-lg rounded-lg w-11/12 sm:w-10/12 md:w-7/12 lg:w-7/12">
        <div class="card-body flex flex-col items-center text-center p-4">
            @if (strpos($user->imagen, 'image/') !== false)
                <img src="{{ asset($user->imagen) }}" class="rounded-circle mb-2" style="width: 100px; height: 100px; object-fit: cover; border-radius: 50%;">
            @else
                <!--Imagen para local -->
                <img src="{{ asset('storage/' . $user->imagen) }}" class="rounded-circle mb-2" style="width: 100px; height: 100px; object-fit: cover; border-radius: 50%;">
                <!--
